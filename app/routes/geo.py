...
def geo_lookup():
    ...
    if not zip_code or len(zip_code) != 5:
        return jsonify({"error": "zip_code must be a 5-digit string"}), 400
    ...
    base_bill = float(svc.billing_df.iloc[-1]["total_bill"]) if svc.billing_df is not None else 150.0
    estimate = estimate_county_bill(base_bill, county)
    return jsonify(estimate)


@geo_bp.get("/geo-all-counties")
